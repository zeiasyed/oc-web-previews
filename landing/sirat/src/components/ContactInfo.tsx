    ...
    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/10 space-y-4">
      ...
      {fullAddress && (
        <div className="space-y-3">
          <p className="text-white/80 text-sm">{fullAddress}</p>
          <button
      ...
      {mosque.parkingInfo && (
        <div className="border-t border-white/10 pt-4 mt-4">
          <p className="text-white/60 text-xs uppercase tracking-wide flex items-center gap-1 mb-1">
            <ParkingSquare size={14} />
            Parking
          </p>
          <p className="text-white/80 text-sm">{mosque.parkingInfo}</p>
        </div>
      )}
    </div>
  );
