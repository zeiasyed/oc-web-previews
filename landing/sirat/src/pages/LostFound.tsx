  ...
  return (
    <div className="min-h-screen bg-charcoal pb-20">
      <div className="px-5 pt-12">
        ...
        <p className="text-white/50 text-sm mb-6">Report or find items across all centers</p>
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-white/40 text-sm">No items reported yet</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <span className="text-white/30 text-xs">{getMosqueName(item.mosqueId)}</span>
                      <span className="text-white/30 text-xs">{item.date}</span>
                    </div>
                    <p className="text-white/40 text-xs mt-1">Contact: {item.contactInfo}</p>
                  </div>
                  {item.status !== 'resolved' && (
                    <button
                      onClick={() => markResolved(item.id)}
                      className="text-white/30 hover:text-green-400 p-1 transition-colors"
